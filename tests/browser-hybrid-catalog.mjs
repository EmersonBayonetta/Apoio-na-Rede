...
console.log(await evaluate(`({cards:document.querySelectorAll('article').length,loaded:document.querySelectorAll('[data-hybrid=hybrid]').length===5,checked:window.__metadata})`));
assert.equal(await evaluate(`document.querySelectorAll('article').length`),5);
assert.equal(await evaluate(`document.querySelectorAll('[data-hybrid=hybrid]').length===5`),true);
await evaluate(`window.__denied=true;document.querySelector('.category-tile').click()`);await pause(500);
assert.equal(await evaluate(`document.querySelectorAll('article').length`),5);
assert.equal(await evaluate(`window.__streetControl`),false);
console.log('PASS five hybrid cards; no dependency on Street View; Street View control disabled');
