...
window.fetch=(url,options)=>String(url).includes('overpass')?Promise.resolve({ok:true,json:async()=>({elements:[{type:'node',id:1,lat:-21.39,lon:-42.69,tags:{name:'Escola Municipal',amenity:'school','addr:street':'Rua da Escola','addr:housenumber':'10'}},{type:'node',id:2,lat:-21.391,lon:-42.69,tags:{amenity:'toilets'}},{type:'node',id:3,lat:-21.392,lon:-42.69,tags:{amenity:'toilets',access:'private'}},{type:'node',id:4,lat:-21.393,lon:-42.69,tags:{highway:'bus_stop'}},{type:'node',id:5,lat:-21.394,lon:-42.69,tags:{highway:'bus_stop'}}]})}):originalFetch(url,options);
const Place={searchNearby:async()=>({places:[]}),searchByText:async()=>({places:[]})};
...
for(const [index,count] of [[5,1],[6,1],[7,2]]) {
 await evaluate(`document.querySelectorAll('.category-tile')[${index}].click()`);await pause(700);
 assert.equal(await evaluate("document.querySelectorAll('article').length"),count,'OSM category '+index);
 if(index===6) assert.equal(await evaluate("document.querySelector('article').textContent.includes('Rua da Escola, 10')"),true);
}
console.log('PASS OSM schools keep name/address, public toilets only, distinct unnamed bus stops');
