...
export function PlaceResultCard({ place, establishment, addressLabel, streetViewScene }: { streetViewScene: StreetViewScene; place?: NearbyPlace; establishment?: Establishment; addressLabel?: string }) {
  ...
  return <article className="premium-card rounded-2xl overflow-hidden" aria-label={addressLabel ? 'Endereço selecionado' : name}>
    <StreetViewPreview scene={streetViewScene} name={name} />
    <p className="px-5 pt-2 text-xs text-slate-500">Street View: vista externa do endereço, que pode estar desatualizada.</p>
